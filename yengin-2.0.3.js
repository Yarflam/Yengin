var yengin = (function () {
	...
	*
	*	> Modified on 16/06/2016 - Yarflam
	*/
	...
	*/
	...
	self.istype = function (obj, type, simple) {
		if(self.isset(simple) && simple) {
			return (self.isset(obj) && (typeof obj == type) ? true : false);
		} else {
			...
			obj.fullscreen = function (fct) {
				if(this.requestFullscreen) {
					...
					if(self.isset(fct)) { self.addEvent('fullscreenchange', document, function trigger () {
						if(!document.webkitIsFullScreen) { self.remEvent('fullscreenchange', document, trigger);fct(); }
					}, false); }
	...
	*/
	...
	return self.deepCopy(self, self.getObj);
})();